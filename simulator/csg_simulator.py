...
class CSGSimulator:
    ...
    def __init__(self, resolution=32, max_steps=512, k_init=5.0, target_shape=None):
        ...
        # ---- Rendering state ----
        # current_step tells the renderer which stock slot and which tool
        # position to draw. After apply_cut(t), call set_current_step(t+1).
    ...
    def tool_sdf(self, p, t):
        """Smoothed capped-cylinder SDF for the tool at trajectory index t."""
        ...
        dx_ = p.x - tp.x
        dy_ = p.y - tp.y
        d_xy = ti.sqrt(dx_ * dx_ + dy_ * dy_ + 1e-8) - r
    ...
    def forward(self, num_active_steps):
        ...
        self.init_stock()
        for t in range(num_active_steps):
            self.apply_cut(t)
        self.compute_loss(num_active_steps)
   ...
   # ========================================================================
    ...
    def tool_sdf_sharp(self, p, t):
        ...
        dx_ = p.x - tp.x
        dy_ = p.y - tp.y
        d_xy = ti.sqrt(dx_ * dx_ + dy_ * dy_ + 1e-12) - r
 
        z_center = tp.z + 0.5 * h
        d_z = ti.abs(p.z - z_center) - 0.5 * h
 
        # Standard exact SDF of a capped cylinder
        d_xy_pos = ti.max(d_xy, 0.0)
        d_z_pos = ti.max(d_z, 0.0)
        outside = ti.sqrt(d_xy_pos * d_xy_pos + d_z_pos * d_z_pos + 1e-12)
        inside = ti.min(ti.max(d_xy, d_z), 0.0)
        return outside + inside
