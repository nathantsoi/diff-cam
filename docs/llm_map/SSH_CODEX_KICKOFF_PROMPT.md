...
2. Review the current `autoresearch` branch and the earlier branches named in the handoff. Use commit graphs, merge bases, diffs, code, and experiment documents—not branch names or commit messages alone. Write `docs/llm_map/BRANCH_HISTORY_REVIEW.md`, distinguishing verified results from claims that lack raw artifacts.
