#!/bin/bash
# Block until all 8 wave-5 runs print a final summary (^hard_dice:) or die.
cd "$(dirname "$0")"
logs=(w5_box_md w5_box_rand w5_pyr_md w5_pyr_rand w5_hole_md w5_hole_rand w5_bowl_md w5_bowl_rand)
declare -A done
while true; do
  alldone=1
  for f in "${logs[@]}"; do
