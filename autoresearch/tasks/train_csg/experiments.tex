...
fixed-$k$ ($k{=}10$) baseline on the $1~\mathrm{in}$ stock, with the initial
(iteration-0) Dice of the random trajectory as a reference. The shape-blind
...
geometry. The iteration-0 Dice is the deployable Dice of the initial random
trajectory before any gradient step; it is $k$-independent (the hard carve used
for evaluation does not depend on $k$), so it is the same for the base and full
columns and reflects only how much of the part the random init happens to cover.
