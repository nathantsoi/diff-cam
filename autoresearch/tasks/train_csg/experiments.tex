...
\paragraph{Shapes.}
We evaluate on four primitives --- \emph{sphere}, \emph{cylinder}, \emph{box},
\emph{square pyramid} --- and two combined-CSG targets that test negative-feature
machining: a \emph{through-hole sphere} $S_{\mathrm{sph}} \cap \neg
S_{\mathrm{cyl}}$ and a \emph{concave bowl} $S_{\mathrm{sph}} \cap \neg
(S_{\mathrm{sub}} \cap \{z \le z_c\})$. All targets are exposed to the method
...
truncation) for all primitive results. All runs are fully deterministic given the
seed.
...
scored near $0.8$ at $k{=}10$ and gain correspondingly less.
...
the concave-bowl combined-CSG target, where it lifts Dice by $\sim 0.03$ over
...
\subsection{Combined-CSG Targets}
\label{sec:combined}

Table~\ref{tab:combined} reports the combined-CSG targets. The concave bowl is
machinable: its negative feature is a wide interior cavity that the swept
cylinder can reach, and the full method scores $0.659$ reproducibly (3-seed
std $0.001$ with the denser $T{=}256$ trajectory; $0.634 \pm 0.030$ at
$T{=}128$). Loss-shift is neutral on the bowl: its concavity is not
over-erosion-limited.

The through-hole sphere is the open frontier. With the default geometry the
through-hole shell ($1.9~\mathrm{mm}$) is thinner than the tool ($3.175~\mathrm{mm}$),
physically infeasible; with a feasible shell the soft optimum trains to
$\approx 0.72$ Dice but deploys at only $\approx 0.25$: at any finite $k$,
soft-union over-erosion fills the narrow negative feature, so the loss cannot
penalize the hole interior. Loss-shift helps marginally ($0.237 \to 0.246$) but
the hole interior remains structurally unpenalized --- the limit of pure
soft-loss methods.

\begin{table}[t]
  \centering
  \caption{Combined-CSG targets, $1~\mathrm{in}$ stock. Deployable Dice.}
  \label{tab:combined}
  \small
  \begin{tabular}{lcc}
    \toprule
    Target & Deployable Dice & Note \\
    \midrule
    Concave bowl ($T{=}256$, rf) & $0.659 \pm 0.001$ & 3 seeds; negative feature reachable \\
    Concave bowl ($T{=}128$)     & $0.634 \pm 0.030$ & ls-0.5 neutral on bowl \\
    Through-hole sphere (feasible) & $0.246$ & soft/hard gap on narrow hole; open \\
    \bottomrule
  \end{tabular}
\end{table}
